...
class SignHiera(nn.Module):
    ...
    def extract_features_fused(
        self,
        x: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        padding: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        """Extract features using a fused Triton kernel for the final reroll + spatial mean pool.

        This avoids materializing the full [B, T, H, W, C] intermediate tensor
        and also skips storing earlier intermediate tensors, saving significant memory.
        """
        from .triton_kernels import fused_spatial_mean_pool
        ...
        # x shape: [B, N, C] where N = T * H_last * W_last
        # Fused spatial mean pool: [B, N, C] -> [B, T, C] without materializing [B, T, H, W, C]
        _, size = self.reroll.schedule[self.stage_ends[-1]]
        T_out = size[0]
        S_out = math.prod(size[1:])  # H * W
        x = fused_spatial_mean_pool(x, T_out, S_out)
    ...
    def from_clip_model(cls, model_id: str, clip_model_path: str) -> nn.Module:
        ...
        raise NotImplementedError(
            "CLIP-based weight conversion is not included in this standalone MAE extraction."
        )
        ...
        if len(frames) > self.config.max_batch_size:
            ...
            for frames_shard, padding_shard in zip(frame_shards, padding_shards):
                with torch.amp.autocast(
                    ...
                    enabled=self.device.type == "cuda" and self._autocast_dtype() is not None,
                ):
