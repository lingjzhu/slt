...
class Prefetcher:
    ...
    def __next__(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if batch is not None and self.use_cuda:
...
def setup_for_distributed(is_main_process: bool) -> None:
    ...
    def print(*args, **kwargs):
        ...
        if is_main_process or force:
            if file not in (None, sys.stdout, sys.stderr) or end != "\n" or flush:
                ORIGINAL_PRINT(*args, file=file, end=end, flush=flush, **kwargs)
            elif len(args) == 1:
                logger.info(args[0], **kwargs)
            else:
                logger.info(" ".join(str(arg) for arg in args), **kwargs)
...
def use_bf16(cfg: DictConfig) -> bool:
    return bool(getattr(cfg.common, "bf16", False))


def use_fp16(cfg: DictConfig) -> bool:
    return bool(getattr(cfg.common, "fp16", False))


def get_autocast_dtype(cfg: DictConfig) -> Optional[torch.dtype]:
    if use_bf16(cfg):
