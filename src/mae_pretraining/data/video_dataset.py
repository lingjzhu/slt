...
from ..utils.video import (get_num_padding_frames, get_start_end_idx,
                           horizontal_flip, random_crop, temporal_sampling,
                           tensor_normalize, uniform_crop)
...
class VideoDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if video_backend == "torchcodec":
            if not _HAS_TORCHCODEC:
                warnings.warn(
                    "`torchcodec` is not installed. Falling back to `pyav` backend."
                )
                video_backend = "pyav"
            else:
                print(f"Using video_backend `torchcodec`")
                self.video_backend = video_backend
        if video_backend == "decord":
            if not _HAS_DECORD:
                warnings.warn(
                    "`decord` is not installed. Falling back to `pyav` backend."
                )
                video_backend = "pyav"
            else:
                print(f"Using video_backend `decord`")
                self.video_backend = video_backend
        ...
        elif video_backend not in ("torchcodec", "decord"):
            assert (
    ...
    def _construct_loader(self):
        ...
        manifest_dir = os.path.join(self.data_dir, "manifests")
        manifest_stem = tsv_file_name[self.mode]
        manifest_candidates = [
            os.path.join(manifest_dir, f"{manifest_stem}.tsv"),
            os.path.join(manifest_dir, "final_train_le16.tsv"),
            os.path.join(manifest_dir, "paired_manifest.tsv"),
        ]
        path_to_manifest = next((p for p in manifest_candidates if pathmgr.exists(p)), None)
        path_to_videos = os.path.join(self.data_dir, "videos")

        assert path_to_manifest is not None, f"No manifest found in {manifest_dir}"
        ...
        with pathmgr.open(path_to_manifest, "r") as f:
            for video_idx, line in enumerate(f):
                ...
                try:
                    fields = line.rstrip("\n").split("\t")
                    if len(fields) < 2:
                        raise ValueError("Manifest row must contain at least video path and duration")
                    video_name = fields[0]
    ...
    def _decode_decord(self, path: str):
        """Decode video using decord. Returns (frames_THWC, fps)."""
        vr = _decord.VideoReader(path, num_threads=1)
        fps = vr.get_avg_fps()
        if not fps:
            raise ValueError("Could not determine fps from decord")
        max_frames = int(self.max_duration * fps) if self.max_duration < math.inf else None
        if max_frames is not None and max_frames < len(vr):
            frames = vr[:max_frames]
        else:
            frames = vr[:]
        # decord with torch bridge returns [T, H, W, C]
        return frames, fps

    def _decode_torchcodec(self, path: str):
        """Decode video using torchcodec. Returns (frames_THWC, fps)."""
        decoder = _TorchCodecDecoder(path, dimension_order="NHWC")
        fps = decoder.metadata.average_fps
        if not fps:
            raise ValueError("Could not determine fps from torchcodec metadata")
        max_frames = int(self.max_duration * fps) if self.max_duration < math.inf else None
        if max_frames is not None and max_frames < decoder.metadata.num_frames:
            frames = decoder[:max_frames]
        else:
            frames = decoder[:]
        # torchcodec returns [T, H, W, C] with NHWC dimension_order
        return frames, fps

    def _decode_torchvision(self, path: str):
        """Decode video using torchvision VideoReader. Returns (frames_THWC, fps)."""
        if self.video_backend == "cuda":
            torch.cuda.set_device(self.gpu)
        set_video_backend(self.video_backend)
        reader = torchvision.io.VideoReader(path)
        metadata = reader.get_metadata()
        fps_values = metadata.get("video", {}).get("fps")
        if not fps_values:
    ...
    def __getitem__(self, index):
        try:
            if self.video_backend == "decord":
                frames, fps = self._decode_decord(self.path_to_videos[index])
            elif self.video_backend == "torchcodec":
                frames, fps = self._decode_torchcodec(self.path_to_videos[index])
            else:
                frames, fps = self._decode_torchvision(self.path_to_videos[index])
            ...
            print(
                f"Skipping malformed video at index {index}: {self.path_to_videos[index]} ({exc})",
                flush=True,
            )
            return None
