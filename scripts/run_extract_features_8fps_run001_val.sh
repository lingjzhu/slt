...
OUT=/mnt/data4/all_val_plain_v3_mae_features_run_001_8fps
SRC=/mnt/data4/all_val_plain_v3
...
src = Path("/mnt/data4/all_val_plain_v3/manifests/paired_manifest.tsv")
out = Path("/mnt/data4/all_val_plain_v3_mae_features_run_001_8fps/shards")
...
for i in range(4):
    ...
    if not videos_link.exists():
        videos_link.symlink_to(Path("/mnt/data4/all_val_plain_v3/videos"), target_is_directory=True)
    handles.append((shard / "manifests" / "val.tsv").open("w"))
...
print(f"Created 4 val shard manifests with {kept} videos")
PY

for gpu in 0 1 2 3; do
    csv_name="val_feature_gpu${gpu}.csv"
    log_file="$OUT/logs/val_gpu${gpu}.log"
    pid_file="$OUT/logs/val_gpu${gpu}.pid"
    ...
    echo $! > "$pid_file"
    echo "Launched val GPU $gpu pid $(cat "$pid_file")"
done

status=0
for gpu in 0 1 2 3; do
    pid=$(cat "$OUT/logs/val_gpu${gpu}.pid")
    if ! wait "$pid"; then
        echo "Val GPU $gpu failed" >&2
        status=1
    fi
done

if [ "$status" -ne 0 ]; then
    echo "One or more val GPU workers failed" >&2
    exit "$status"
fi
...
out = Path("/mnt/data4/all_val_plain_v3_mae_features_run_001_8fps")
src_manifest = Path("/mnt/data4/all_val_plain_v3/manifests/paired_manifest.tsv")
shard_csvs = [out / f"val_feature_gpu{i}.csv" for i in range(4)]
merged_csv = out / "val_feature.csv"
val_feat_tsv = out / "manifests" / "val_features.tsv"
detail_tsv = out / "manifests" / "val_feature_detailed.tsv"
...
with merged_csv.open() as fcsv, \
    val_feat_tsv.open("w", newline="") as fval, \
    detail_tsv.open("w", newline="") as fdetail:
    reader = csv.DictReader(fcsv)
    val_writer = csv.writer(
        fval, delimiter="\t", quoting=csv.QUOTE_NONE, escapechar="\\"
    )
    detail_fields = [
        "video_path",
        "feature_path",
    ...
    for idx, (row, parts) in enumerate(zip(reader, manifest_rows), start=1):
        ...
        val_writer.writerow([video_name, row["num_feature_vectors"], caption])
...
print("Wrote val_features.tsv and val_feature_detailed.tsv")
PY
