...
src = Path("/mnt/data4/all_train_plain_v3/manifests/paired_manifest.tsv")
out = Path("/mnt/data4/all_train_plain_v3_mae_features_run_001_8fps/shards")
...
print(f"Created 4 shard manifests with {kept} videos; skipped {skipped} videos > 30s")
PY

for gpu in 0 1 2 3; do
    csv_name="train_feature_gpu${gpu}.csv"
    log_file="$OUT/logs/gpu${gpu}.log"
    pid_file="$OUT/logs/gpu${gpu}.pid"
    ...
    echo $! > "$pid_file"
    echo "Launched GPU $gpu pid $(cat "$pid_file")"
done

status=0
for gpu in 0 1 2 3; do
    pid=$(cat "$OUT/logs/gpu${gpu}.pid")
    if ! wait "$pid"; then
        echo "GPU $gpu failed" >&2
        status=1
    fi
done
...
out = Path("/mnt/data4/all_train_plain_v3_mae_features_run_001_8fps")
src_manifest = Path("/mnt/data4/all_train_plain_v3/manifests/paired_manifest.tsv")
shard_csvs = [out / f"train_feature_gpu{i}.csv" for i in range(4)]
merged_csv = out / "train_feature.csv"
train_feat_tsv = out / "manifests" / "train_features.tsv"
detail_tsv = out / "manifests" / "train_feature_detailed.tsv"
...
with src_manifest.open() as fsrc:
    for line in fsrc:
        ...
        if len(parts) < 2:
            continue
        if float(parts[1]) <= 30.0:
            filtered_manifest_rows.append(parts)
...
with merged_csv.open() as fcsv, \
    train_feat_tsv.open("w", newline="") as ftrain, \
    detail_tsv.open("w", newline="") as fdetail:
    reader = csv.DictReader(fcsv)
    train_writer = csv.writer(
        ftrain, delimiter="\t", quoting=csv.QUOTE_NONE, escapechar="\\"
    )
    detail_fields = [
        "video_path",
        "feature_path",
    ...
    for idx, (row, parts) in enumerate(zip(reader, filtered_manifest_rows), start=1):
        if len(parts) < 5:
            raise RuntimeError(f"Invalid source manifest row at {idx}")
        ...
        train_writer.writerow([video_name, row["num_feature_vectors"], caption])
...
print("Wrote train_features.tsv and train_feature_detailed.tsv")
PY
