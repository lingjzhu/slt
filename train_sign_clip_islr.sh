...
export SIGN_CLIP_LOG_DDP_PARAM_INDEXES="${SIGN_CLIP_LOG_DDP_PARAM_INDEXES:-0}"
...
ARGS=(
  ...
  --output-dir "${OUTPUT_DIR:-$SCRIPT_DIR/outputs/sign_clip_islr_random}"
  --train-steps "${TRAIN_STEPS:-20000}"
  --eval-every "${EVAL_EVERY:-200000}"
  ...
  --target-fps "${TARGET_FPS:-15}"
  --crop-size "${CROP_SIZE:-224}"
  --max-text-length "${MAX_TEXT_LENGTH:-16}"
  --mixed-precision "${MIXED_PRECISION:-bf16}"
  --equal-dataset-mix
  --wandb-project "${WANDB_PROJECT:-sign-clip-islr}"
)
if [[ -n "${HIERA_CKPT:-}" ]]; then
  ARGS+=(--hiera-ckpt "$HIERA_CKPT")
fi
...
if [[ "$NUM_GPUS" -gt 1 ]]; then
  echo "Launching DDP sign-clip training on $NUM_GPUS GPUs"
  exec torchrun --standalone --nproc_per_node="$NUM_GPUS" --master_port="$MASTER_PORT" "${ARGS[@]}"
else
  echo "Launching single-process sign-clip training"
  exec python "${ARGS[@]}"
fi
